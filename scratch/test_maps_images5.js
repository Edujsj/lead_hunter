...
async function test() {
  ...
  if (name) {
    ...
    if (await link.count() > 0) {
      ...
      console.log("link not found! falling back to text click");
      await page.locator(`[role="feed"] [class*="fontHeadlineSmall"]:has-text("${safeName.slice(0, 30)}")`).first().click({ timeout: 4000, force: true });
    }
    
    await page.waitForTimeout(4000);

    await page.screenshot({ path: 'scratch/panel2.png' });
